...
class DialogDataset(data.Dataset):
    def __init__(self, path, class2id, word2id, key, roberta=False, pretrain_path = None, loss_mask = None):
        ...
        _, _, _, _, self.videoAudio, self.videoVisual, _, _, _ = pickle.load(
            open("conv-emotion-master-2/DialogueGCN/data/iemocap/IEMOCAP_features.pkl", 'rb'), encoding='latin1')
    ...
    def __getitem__(self, index):
        ...
        video = self.videoVisual[self.ids[index]]
        audio = self.videoAudio[self.ids[index]]
        if self.mask_data is not None:
            mask = self.mask_data[index]
            return [utterance, label, mask]
        return [utterance, label, video, audio]
    ...
    def collate_fn(self, data):
        if self.mask_data is not None:
            ...
            utterance, label, video, audio = data[0]
        ...
        if self.mask_data is not None:
            ...
            return [pad_tensor, label, lens, torch.tensor(video), torch.tensor(audio)]
